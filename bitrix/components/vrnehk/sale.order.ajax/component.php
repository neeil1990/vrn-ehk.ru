...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		$arSelFields = array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY",
			...
			"NOTES", "DISCOUNT_PRICE", "PRODUCT_PROVIDER_CLASS","PRICE_TYPE_ID","MEASURE_CODE","MEASURE_NAME", "DIMENSIONS", "TYPE", "SET_PARENT_ID", "DETAIL_PAGE_URL"
		);
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				if($arResult["ORDER_ID"]){
					...
					$discount->calculate();
					 ...
					 if($arOrderDat['ORDER_PROP'][15]){
						$fio_fise = getPropertyByCode($propertyCollection, 'F_I_O_FISFASE');
						$fio_fise->setValue($arOrderDat['ORDER_PROP'][15]);
					 }

					 if($arOrderDat['ORDER_PROP'][16]){
						$serial_pass = getPropertyByCode($propertyCollection, 'SERIAL_PASSPORT_FISFACE');
						$serial_pass->setValue($arOrderDat['ORDER_PROP'][16]);
					 }

					if($arOrderDat['ORDER_PROP'][26]){
						$serial_pass = getPropertyByCode($propertyCollection, 'TRANSPORT_COMPANIES');
						$serial_pass->setValue($arOrderDat['ORDER_PROP'][26]);
					}

					if($arOrderDat['ORDER_PROP'][27]){
						$serial_pass = getPropertyByCode($propertyCollection, 'CDEK_ADRESS');
						$serial_pass->setValue($arOrderDat['ORDER_PROP'][27]);
					}

					if($arOrderDat['ORDER_PROP'][29]){
						$serial_pass = getPropertyByCode($propertyCollection, 'TRANSPORT_COMPANIES');
						$serial_pass->setValue($arOrderDat['ORDER_PROP'][29]);
					}

					if($arOrderDat['ORDER_PROP'][30]){
						$serial_pass = getPropertyByCode($propertyCollection, 'CDEK_ADRESS');
						$serial_pass->setValue($arOrderDat['ORDER_PROP'][30]);
					}
				...
				if (empty($arResult["ERROR"]))
				{
					...
					foreach ($arBasketList as $arItem)
					{
						...
						$dbBasketProps = CSaleBasket::GetPropsList(
					        ...
					        array("BASKET_ID" => $arItem["ID"], "CODE"=> "BASE_UNIT")
					    );
						$prop = $dbBasketProps->Fetch();

						$measureText = (isset($arItem["MEASURE_TEXT"]) && strlen($arItem["MEASURE_TEXT"])) ? $arItem["MEASURE_TEXT"] : GetMessage("SOA_SHT");

						$strOrderList .= $arItem["NAME"]." - ".$arItem["QUANTITY"]." ".$prop["VALUE"].". ".": ".SaleFormatCurrency($arItem["PRICE"], $arItem["CURRENCY"]);
						$strOrderList .= "\n";
					}
