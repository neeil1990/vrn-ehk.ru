...
<div class="bx_section">
	...
	if(!empty($arResult["DELIVERY"]))
	{
		...
		?>
		<h4><?=GetMessage("SOA_TEMPL_DELIVERY")?>
		...
		<div class="delivery_list">
			...
			foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
			{
				...
				else // stores and courier
				{
					...
					?>
						<div class="bx_block w100 vertical change_delivery <?if($count==1){echo "first";}?>">
							...
							<div class="bx_element">
								...
								<label for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>" <?=$clickHandler?>>
									...
									<div class="delivery_sides">
										<div class="logo">
											<img <?=My::NewResize($arDelivery["LOGOTIP"]["ID"],90,1000,false)?> alt="<?=htmlspecialcharsbx($arDelivery["NAME"])?>" >
										</div>
										<div class="info">
											<?if($arDelivery["PRICE"]>0)
											{?>
												<div class="text">Стоимость: <?=My::Money($arDelivery["PRICE"]);?> руб.</div>
											<?}
											if (strlen($arDelivery["DESCRIPTION"])>0)
											{?>
												<div class="add"><?=htmlspecialchars_decode($arDelivery["DESCRIPTION"])?></div>
											<?}?>
										</div>
										<div class="clear"></div>
									</div>
