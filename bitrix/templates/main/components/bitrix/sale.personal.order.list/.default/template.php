							...
							<tr>
									...
									<ul class="bx_item_list">
										...
										foreach ($order["BASKET_ITEMS"] as $item):?>
											<li>
												...
												<?endif?>
												<nobr>&nbsp;&mdash; <?=$item['QUANTITY']?> <?=$item["BASE_UNIT"]?>. <?//=(isset($item["MEASURE_NAME"]) ? $item["MEASURE_NAME"] : GetMessage('SPOL_SHT'))?></nobr>
											</li>
								...
								<td>
									...
									<?if($order["ORDER"]["CANCELED"] != "Y"):?>
										<a href="<?=$order["ORDER"]["URL_TO_CANCEL"]?>" style="min-width:140px"class="bx_big bx_bt_button_type_2 bx_cart bx_order_action cancel"><?=GetMessage('SPOL_CANCEL_ORDER')?></a>
									<?endif?>
									<?
									$date1=$order["ORDER"]["DATE_STATUS_FORMATED"];
									$date2= date("d.m.Y", mktime(0, 0, 0, date('m'), date('d') - 7, date('Y')));
									if(strtotime($date1)>strtotime($date2) && $order["ORDER"]["PAYED"] != "Y" && $arResult["INFO"]["PAY_SYSTEM"][$order["ORDER"]["PAY_SYSTEM_ID"]]["NAME"]=="Uniteller"){
										?>
										<a data-popup='pay-user' href="/make-order/?ORDER_ID=<?=$order["ORDER"]["ID"]?>&pay=user" style="min-width:140px"class="bx_big bx_bt_button_type_2 bx_cart bx_order_action">Оплатить</a>
										<?
									}
									?>
...
<div id="pay-user"></div>
<script>
	$(document).on("click", "[data-popup='pay-user']", function () {
