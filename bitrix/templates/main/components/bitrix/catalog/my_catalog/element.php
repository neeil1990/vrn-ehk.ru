...
if (CModule::IncludeModule('iblock')) {
   ...
   $dbElements1 = CIBlockElement::GetList(array(), $arFilter, false, false, $arSelect);;
   $dbElements1->SetUrlTemplates('/catalog/#SECTION_CODE#/#ELEMENT_ID#/');
   while ($arElement = $dbElements1->GetNext()) {
      $arElements[$arElement['ID']]['old_url'] = $arElement['DETAIL_PAGE_URL'];
   }
   
   $dbElements2 = CIBlockElement::GetList(array(), $arFilter, false, false, $arSelect);;
   $dbElements2->SetUrlTemplates('/catalog/#SECTION_CODE#/#ELEMENT_CODE#/');
   while ($arElement = $dbElements2->GetNext()) {
      $arElements[$arElement['ID']]['new_url'] = $arElement['DETAIL_PAGE_URL'];
   }
   
   foreach($arElements as $key => $redirect){
	   
	  $id_url = explode('/',$APPLICATION->GetCurPageParam());
	  
	   if($key == $id_url[3]){
		   
		   LocalRedirect($redirect['new_url'], true, 301);
	   
	   }else{
	  //var_dump($redirect['old_url']);
	   
	   if($redirect['old_url'] == $APPLICATION->GetCurPageParam()){
