...
(function (exports,main_core_events,calendar_util,main_core,calendar_ui_tools_draganddrop,main_popup) {
	...
	class Selector extends main_core_events.EventEmitter {
	  ...
	  update(params = {}) {
	    ...
	    if (main_core.Type.isDate(from) && main_core.Type.isDate(to)) {
	      ...
	      this.currentDateTo = to; // Update selector
	  ...
	  move(x) {
	    if (this.selectorIsDraged) {
	      let pos = this.selectorStartLeft + x; // Correct cursor position acording to changes of scrollleft
	...
	class Planner extends main_core_events.EventEmitter {
	  ...
	  build() {
	    ...
	    let entriesListWidth = this.compactMode ? 0 : this.entriesListWidth; // Timeline with accessibility information
	  ...
	  extendTimeline(extendedTimeFrom, extendedTimeTo, isToRight = false) {
	    ...
	    this.DOM.timelineVerticalConstraint.scrollLeft = scroll + cellsInsertedOnLeftCount * this.timelineCellWidth; // get accessibility events for animation
	    ...
	    this.update(this.entries, this.accessibility); // update selector and visible events position during the css animation
	  ...
	  checkTimelineScroll() {
	    ...
	    const maxScroll = this.DOM.timelineVerticalConstraint.scrollWidth - this.DOM.timelineFixedWrap.offsetWidth - this.scrollStep; // Check and expand only if it is visible
	  ...
	  update(entries = [], accessibility = {}) {
	    ...
	    const userId = parseInt(this.userId); // sort entries list by amount of accessibility data
