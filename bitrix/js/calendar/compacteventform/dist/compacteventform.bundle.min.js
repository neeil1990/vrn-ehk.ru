this.BX=this.BX||{};(function(e,t,i,s,n,r,a,o){"use strict";let l=e=>e,c,d,h,u,f,p,g,E,m,y,D,C,M,S,v,T,I,O,w,b,_,L,N,A,k,B,P,V,R;class U extends i.EventEmitter{constructor(e={}){super();this.STATE={READY:1,REQUEST:2,ERROR:3};this.zIndex=1200;this.Z_INDEX_OFFSET=-1e3;this.userSettings="";this.DOM={};this.displayed=false;this.sections=[];this.sectionIndex={};this.trackingUsersList=[];this.checkDataBeforeCloseMode=true;this.CHECK_CHANGES_DELAY=500;this.RELOAD_DATA_DELAY=500;this.setEventNamespace("BX.Calendar.CompactEventForm");this.userId=e.userId||s.Util.getCurrentUserId();this.type=e.type||"user";this.isLocationCalendar=e.isLocationCalendar||false;this.calendarContext=e.calendarContext||null;this.ownerId=e.ownerId||this.userId;this.BX=s.Util.getBX();this.checkForChangesDebounce=t.Runtime.debounce(this.checkForChanges,this.CHECK_CHANGES_DELAY,this);this.reloadEntryDataDebounce=t.Runtime.debounce(this.reloadEntryData,this.RELOAD_DATA_DELAY,this);this.checkOutsideClickClose=this.checkOutsideClickClose.bind(this);this.outsideMouseDownClose=this.outsideMouseDownClose.bind(this);this.keyHandler=this.handleKeyPress.bind(this)}show(e=U.EDIT_MODE,i={}){this.setParams(i);this.setMode(e);this.state=this.STATE.READY;this.popupId="compact-event-form-"+Math.round(Math.random()*1e5);if(this.popup){this.popup.destroy()}this.popup=this.getPopup(i);t.Dom.addClass(this.popup.titleBar,"calendar-add-popup-titlebar");t.Dom.removeClass(this.popup.popupContainer,"popup-window-with-titlebar");t.Dom.removeClass(this.popup.closeIcon,"popup-window-titlebar-close-icon");t.Event.bind(document,"mousedown",this.outsideMouseDownClose);t.Event.bind(document,"mouseup",this.checkOutsideClickClose);t.Event.bind(document,"keydown",this.keyHandler);t.Event.bind(this.popup.popupContainer,"transitionend",(()=>{t.Dom.removeClass(this.popup.popupContainer,"calendar-simple-view-popup-show")}));if(this.getMode()===U.EDIT_MODE){a.EntryManager.doDelayedActions()}this.prepareData().then((()=>{if(this.checkLocationView()){this.setFormValuesLocation()}else{this.setFormValues()}this.popup.show();this.checkDataBeforeCloseMode=true;if(this.canDo("edit")&&this.DOM.titleInput&&e===U.EDIT_MODE){this.DOM.titleInput.focus();this.DOM.titleInput.select()}this.displayed=true;if(this.getMode()===U.VIEW_MODE){s.Util.sendAnalyticLabel({calendarAction:"view_event",formType:"compact"});this.popup.getButtons()[0].button.focus()}if(this.getMode()===U.EDIT_MODE&&!this.userPlannerSelector.isPlannerDisplayed()){this.userPlannerSelector.checkBusyTime()}}))}checkLocationView(){return this.getMode()===U.VIEW_MODE&&this.type==="location"}getPopup(e){return new n.Popup(this.popupId,e.bindNode,{zIndex:this.zIndex+this.Z_INDEX_OFFSET,closeByEsc:true,offsetTop:0,offsetLeft:0,closeIcon:true,titleBar:true,draggable:true,resizable:false,lightShadow:true,className:"calendar-simple-view-popup calendar-simple-view-popup-show",cacheable:false,content:this.checkLocationView()?this.getPopupContentLocation():this.getPopupContentCalendar(),buttons:this.getButtons(),events:{onPopupClose:this.close.bind(this)}})}isShown(){return this.displayed}close(){if(this.getMode()===U.EDIT_MODE&&this.formDataChanged()&&this.checkDataBeforeCloseMode&&!confirm(t.Loc.getMessage("EC_SAVE_ENTRY_CONFIRM"))){if(this.popup){this.popup.destroyed=true;setTimeout((()=>{this.popup.destroyed=false}),0)}return}this.displayed=false;this.emit("onClose");t.Event.unbind(document,"mousedown",this.outsideMouseDownClose);t.Event.unbind(document,"mouseup",this.checkOutsideClickClose);t.Event.unbind(document,"keydown",this.keyHandler);if(this.userPlannerSelector){this.userPlannerSelector.destroy()}if(this.popup){this.popup.destroy()}if(r.Location){r.Location.setCurrentCapacity(0)}s.Util.clearPlannerWatches();s.Util.closeAllPopups()}getPopupContentCalendar(){this.DOM.wrap=t.Tag.render(c||(c=l`<div class="calendar-add-popup-wrap">
		...
		</div>`),this.DOM.titleOuterWrap=t.Tag.render(d||(d=l`
			<div class="calendar-field-container calendar-field-container-string-select">
				<div class="calendar-field-block">
					${0}
					${0}
				</div>
			</div>`),this.getTitleControl(),this.getColorControl()),this.getSectionControl("textselect"),this.getDateTimeControl(),this.getUserPlannerSelector(),this.getTypeInfoControl(),this.getLocationControl(),this.DOM.remindersOuterWrap=t.Tag.render(h||(h=l`
				<div class="calendar-field-block">
					<div class="calendar-field-title">${0}:</div>
					${0}
				</div>`),t.Loc.getMessage("EC_REMIND_LABEL"),this.createRemindersControl()),this.getRRuleInfoControl());return this.DOM.wrap}getPopupContentLocation(){this.DOM.wrap=t.Tag.render(u||(u=l`<div class="calendar-add-popup-wrap">
			${0}
			<div class="calendar-field-container calendar-field-container-choice">
				${0}
			</div>
		...
		`),t.Loc.getMessage("EC_ENTRY_NAME"));t.Event.bind(this.DOM.titleInput,"keyup",this.checkForChangesDebounce);t.Event.bind(this.DOM.titleInput,"change",this.checkForChangesDebounce);return this.DOM.titleInput}getTitleControlLocation(){this.DOM.titleInput=t.Tag.render(E||(E=l`
			<input class="calendar-field calendar-field-string"
				...
				readonly
			/>
		`),t.Loc.getMessage("EC_ENTRY_NAME"));return this.DOM.titleInput}getHostControl(){const e=this.entry.data.CREATED_BY;const i=U.USER_URL.replace("#USER_ID#",e);const s=this.BX.Calendar.EntryManager.userIndex[e]?this.BX.Calendar.EntryManager.userIndex[e].AVATAR:"";this.DOM.hostBar=t.Tag.render(m||(m=l`
		...
		`),t.Loc.getMessage("EC_HOST")+": ",i,s,i,BX.util.htmlspecialchars(this.entry.name));return this.DOM.hostBar}getColorControl(){this.DOM.colorSelect=t.Tag.render(y||(y=l`<div class="calendar-field calendar-field-select calendar-field-tiny"></div>`));this.colorSelector=new r.ColorSelector({wrap:this.DOM.colorSelect,mode:"selector"});this.colorSelector.subscribe("onChange",(e=>{if(e instanceof i.BaseEvent){const t=e.getData().value;if(!this.isNewEntry()&&(this.canDo("edit")||this.entry.getCurrentStatus()!==false)){this.BX.ajax.runAction("calendar.api.calendarajax.updateColor",{data:{entryId:this.entry.id,userId:this.userId,color:t}});this.entry.data.COLOR=t;this.emit("doRefresh");this.emitOnChange()}}}));return this.DOM.colorSelect}getColorControlsLocationView(){this.DOM.colorSelect=t.Tag.render(D||(D=l`<div class="calendar-field calendar-field-select calendar-colorpicker-readonly calendar-field-tiny"></div>`));this.colorSelector=new r.ColorSelector({wrap:this.DOM.colorSelect,mode:"view"});return this.DOM.colorSelect}getSectionControl(e){this.DOM.sectionSelectWrap=t.Tag.render(C||(C=l`<div class="calendar-field-choice-calendar"></div>`));this.sectionSelector=new r.SectionSelector({outerWrap:this.DOM.sectionSelectWrap,defaultCalendarType:this.type,defaultOwnerId:this.ownerId,sectionList:this.sections,sectionGroupList:o.SectionManager.getSectionGroupList({type:this.type,ownerId:this.ownerId,userId:this.userId,trackingUsersList:this.trackingUsersList}),mode:e,zIndex:this.zIndex,getCurrentSection:()=>{const e=this.getCurrentSection();if(e){return{id:e.id,name:e.name,color:e.color}}return false},selectCallback:e=>{if(e){if(this.colorSelector){this.colorSelector.setValue(e.color)}this.sectionValue=e.id;this.checkForChangesDebounce();o.SectionManager.saveDefaultSectionId(this.sectionValue)}}});return this.DOM.sectionSelectWrap}getDateTimeControl(){this.DOM.dateTimeWrap=t.Tag.render(M||(M=l`<div class="calendar-field-container calendar-field-container-datetime"></div>`));this.dateTimeControl=new r.DateTimeControl(null,{showTimezone:false,outerWrap:this.DOM.dateTimeWrap,inlineEditMode:true});this.dateTimeControl.subscribe("onChange",(e=>{if(e instanceof i.BaseEvent){let t=e.getData().value;if(this.remindersControl){this.remindersControl.setFullDayMode(t.fullDay);if(this.isNewEntry()&&!this.remindersControl.wasChangedByUser()){const e=a.EntryManager.getNewEntryReminders(t.fullDay?"fullDay":"withTime");this.remindersControl.setValue(e,false)}}if(this.userPlannerSelector){if(!this.userPlannerSelector.isPlannerDisplayed()){this.userPlannerSelector.showPlanner()}this.userPlannerSelector.setLocationValue(this.locationSelector.getTextValue());this.userPlannerSelector.setDateTime(t,true);this.userPlannerSelector.refreshPlannerStateDebounce()}if(this.locationSelector){this.locationSelector.checkLocationAccessibility({from:e.getData().value.from,to:e.getData().value.to,fullDay:e.getData().value.fullDay,currentEventId:this.entry.id})}this.checkForChangesDebounce()}}));return this.DOM.dateTimeWrap}getUserPlannerSelector(){this.DOM.userPlannerSelectorOuterWrap=t.Tag.render(S||(S=l`<div>
			...
			${0}
		<div>`),this.DOM.userSelectorWrap=t.Tag.render(v||(v=l`
				<div class="calendar-field-block">
				...
				</div>`),t.Loc.getMessage("EC_ATTENDEES_MORE"),t.Loc.getMessage("EC_SEC_SLIDER_CHANGE")),this.DOM.informWrap=t.Tag.render(T||(T=l`
				<div class="calendar-field-container-inform">
					<span class="calendar-field-container-inform-text">${0}</span>
				</div>`),t.Loc.getMessage("EC_NOTIFY_OPTION")),this.DOM.plannerOuterWrap=t.Tag.render(I||(I=l`
				<div class="calendar-planner-wrapper" style="height: 0">
				</div>`)),this.DOM.hideGuestsWrap=t.Tag.render(O||(O=l`
