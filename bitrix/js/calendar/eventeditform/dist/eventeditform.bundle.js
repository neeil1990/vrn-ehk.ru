...
(function (exports,main_core,calendar_controls,calendar_util,calendar_entry,calendar_sectionmanager,main_core_events,calendar_planner,ui_entitySelector,calendar_roomsmanager) {
	...
	class EventEditForm {
	  ...
	  setFormValues() {
	    let entry = this.entry; // Date time
	    ...
	    this.colorSelector.setValue(this.formDataValue.color || entry.getColor() || section.COLOR); // Reminders

	    this.remindersControl.setValue(this.formDataValue.reminder || entry.getReminders(), true, false); // Recursion

	    this.repeatSelector.setValue(this.formDataValue.rrule || entry.getRrule()); // accessibility
	  ...
	  handlePlannerSelectorChanges(event) {
	    if (event instanceof main_core_events.BaseEvent) {
	      let data = event.getData(); // Date time
