<?php
/**
 * Формирует чек. Посылает запрос в платёжную систему на получание результатов авторизации.
 * На основании результатов этого запроса:
 *  - если нужно: меняет статус заказа в электронном магазине.
 *  - если нужно: посылает запрос на отмену платежа в платёжную систему.
 *  - формирует чек для показа пользователю.
 * Вызывается
 *  - при заходе пользователем на страницу "чек" из платёжной системы.
 *  - при заходе пользователем на страницу "чек" из списка заказов.
 *  - администратором в административной части сайта при просмотре подробностей заказа в разделе "Оплата" при нажатии на ссылку "Получить >>".
 *  - администратором в административной части сайта при просмотре списка заказов при групповой операции "Обновить статусы платёжных систем".
...
// Формирует html для страницы чека.
$html = '';
if ($aCheckData['response_code'] !== '' || $aCheckData['error_message'] !== '') {
	...
	// Если это не страница чека, то ничего не выводить
