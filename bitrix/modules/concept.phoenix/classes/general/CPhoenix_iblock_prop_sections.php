...
define('CPHX_UT_SECTION', 'SCPHXSection');

class CPhoenixIblockPropSection {
	...
	public static function GetUserTypeDescription() {
		return array(
			'PROPERTY_TYPE' => 'N',
			'USER_TYPE' => CPHX_UT_SECTION,
			'DESCRIPTION' => GetMessage('CPHX_UT_SECTION_DESCR'),
		...
		if (isset($value['VALUE']) && !empty($value['VALUE']) && !empty($arProperty['IBLOCK_ID'])) {
			...
			if (isset(self::$cache[$value['VALUE']])) {
				if (self::$cache[$value['VALUE']]['IBLOCK_ID'] != $arProperty['IBLOCK_ID']) {
					$arResult[] = GetMessage('CPHX_UT_SECTION_NO_PARENT_IBLOCK');
				}
			}
		}
		return $arResult;
	...
	public static function GetPropertyFieldHtml($arProperty, $arValue, $strHTMLControlName) {
		...
		if (
			...
			<option value=""><? echo GetMessage('CPHX_UT_SECTION_MESS_TOP_LEVEL'); ?></option>
			<?
	...
	public static function GetSettingsHTML($arFields, $strHTMLControlName, &$arPropertyFields) {
		$arPropertyFields = array(
			'USER_TYPE_SETTINGS_TITLE' => GetMessage('CPHX_UT_SECTION_SETTING_TITLE'),
		);
		$arSettings = self::PrepareSettings($arFields);
		return '<tr>
		<td>'.GetMessage('CPHX_UT_SECTION_SETTING_VIEW_MODE').'</td>
		<td>'.SelectBoxFromArray(
			$strHTMLControlName['NAME'].'[VIEW_MODE]',
			array(
				'REFERENCE' => array(
					GetMessage('CPHX_UT_SECTION_SETTING_VIEW_MODE_SELECT'),
					GetMessage('CPHX_UT_SECTION_SETTING_VIEW_MODE_WINDOW')
				),
				'REFERENCE_ID' => array(
		...
		<tr>
		<td>'.GetMessage('CPHX_UT_SECTION_SETTING_MAX_LEVEL').'</td>
		<td><input type="text" name="'.$strHTMLControlName['NAME'].'[MAX_LEVEL]" value="'.$arSettings['MAX_LEVEL'].'"> '.GetMessage('CPHX_UT_SECTION_SETTING_MAX_LEVEL_DESCR').'</td>
		</tr>';
	}
