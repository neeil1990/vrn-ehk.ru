...
class FacetConceptPhoenix
{
	...
	protected $sectionId = array(0);
	protected $elementId = array();
	...
	public function query(array $filter, array $facetTypes = array(), $facetId = 0)
	{
		...
		//in cache sections and elements is empty
		if ($filter)
		{
			if(!$this->sectionId && $filter["SECTION_IDS"])
			{
				$this->sectionId = $filter["SECTION_IDS"];
				unset($filter["SECTION_IDS"]);
			}
			if(!$this->elementId && $filter["ELEMENT_IDS"])
			{
				$this->elementId = $filter["ELEMENT_IDS"];
				unset($filter["ELEMENT_IDS"]);
			}
		}
		...
		if(!empty($facets))
		{
			foreach ($facets as $facetWhereAndFacets)
			{
				...
				if (empty($where))
				{
					if($this->sectionId && is_array($this->sectionId))
						$sectionWhere = ' AND (F.SECTION_ID IN ('.implode(',', $this->sectionId).'))';
					if($this->elementId && is_array($this->elementId))
					{
						$elementWhereTmp = $elementWhere;
						$elementWhereTmp .= ' AND (F.ELEMENT_ID IN ('.implode(',', $this->elementId).'))';
					}
					...
					$sqlUnion[] = "
						...
						WHERE
							F.FACET_ID in (".implode(",", $facetFilter).")
							".$sectionWhere."
							".$elementWhereTmp."
						GROUP BY
							F.FACET_ID, F.VALUE
				...
				else
				{
					...
					$fcJoin = "
							INNER JOIN (
								...
								WHERE FC0.SECTION_ID IN (".implode(',', $this->sectionId).")
								AND (
				...
				$sqlUnion[] = "
					...
					WHERE
						F.FACET_ID in (".implode(",", $facetFilter).")"
						.(($this->sectionId && is_array($this->sectionId)) ? ' AND (F.SECTION_ID IN ('.implode(',', $this->sectionId).'))' : '')
						.(($this->elementId && is_array($this->elementId)) ? ' AND (F.ELEMENT_ID IN ('.implode(',', $this->elementId).'))' : '')
						."
					GROUP BY
						F.FACET_ID, F.VALUE
				";
	...
	protected function getFacetFilter(array $facetTypes)
	{
		...
		if(!empty($this->sectionId))
		{
			foreach($this->sectionId as $sectionID){
				...
				foreach ($this->getSectionFilterProperty($sectionID) as $propertyId => $propertyType)
				{
	...
	public function whereToSql(array $where, $tableAlias, $subsectionsCondition = "")
	{
		$sqlWhere = "";
		if($this->sectionId)
			$sectionCondition = "$tableAlias.SECTION_ID IN (".implode(',', $this->sectionId).')';
		else
	 ...
	 */
	public function setSectionId($sectionIDs)
	{
	...
	public function setElementId($elementIDs)
	{
