this.BX=this.BX||{};this.BX.Calendar=this.BX.Calendar||{};this.BX.Calendar.Sync=this.BX.Calendar.Sync||{};(function(e,t,s,n,i,a,o,r,c,l,d,h){"use strict";let p=e=>e,u,g,_,C,E;class v{constructor(e){this.status=e.status;this.connections=e.connections;this.withStatusLabel=e.withStatusLabel;this.popupWithUpdateButton=e.popupWithUpdateButton;this.popupId=e.popupId}static createInstance(e){return new this(e)}setStatus(e){this.status=e;return this}setConnections(e){this.connections=e;return this}getContent(){let e;if(this.status==="success"){e=l.Tag.render(u||(u=p`
		...
		`),this.getMainHeader(),this.getStatusBlockContent(this.getConnections()))}getMainHeader(){return this.cache.remember("calendar-syncPanel-mainHeader",(()=>l.Tag.render(L||(L=S`
				<span class="calendar-sync-header-text">${0}</span>
			`),l.Loc.getMessage("SYNC_CALENDAR_HEADER_NEW"))))}getMobileContentWrapper(){return this.cache.remember("calendar-syncPanel-mobileContentWrapper",(()=>l.Tag.render(I||(I=S`
		...
		`))))}getWebContentWrapper(){return this.cache.remember("calendar-syncPanel-webContentWrapper",(()=>l.Tag.render(A||(A=S`
			...
			`))))}getMobileHeader(){return this.cache.remember("calendar-syncPanel-mobileHeader",(()=>l.Tag.render(m||(m=S`
				<div class="calendar-sync-title">${0}</div>
			`),l.Loc.getMessage("SYNC_MOBILE_HEADER"))))}getWebHeader(){return this.cache.remember("calendar-syncPanel-webHeader",(()=>l.Tag.render(N||(N=S`
				<div class="calendar-sync-title">${0}</div>
		`),l.Loc.getMessage("SYNC_WEB_HEADER"))))}getStatusBlockContent(e){this.statusBlock=v.createInstance({status:this.status,connections:e,withStatusLabel:true,popupWithUpdateButton:true,popupId:"calendar-syncPanel-status"});this.statusBlockContent=this.statusBlock.getContent();return this.statusBlockContent}getConnections(){const e=[];const t=Object.values(this.connectionsProviders);t.forEach((t=>{const s=t.getConnections();if(s.length>0){s.forEach((t=>{if(n.ConnectionItem.isConnectionItem(t)&&t.getConnectStatus()===true){e.push(t)}}))}}));return e}setGridContent(){const e=Object.values(this.connectionsProviders);this.showWebGridContent(e.filter((e=>e.mainPanel===false&&e.getViewClassification()==="web")));this.showMobileGridContent(e.filter((e=>e.mainPanel===false&&e.getViewClassification()==="mobile")))}showWebGridContent(e){const t=this.getWebContentWrapper();l.Dom.clean(t);const s=new BX.TileGrid.Grid({id:"calendar_sync",items:e,container:t,sizeRatio:"55%",itemMinWidth:180,tileMargin:7,itemType:"BX.Calendar.Sync.Interface.GridUnit",userId:this.userId});s.draw()}showMobileGridContent(e){const t=this.getMobileContentWrapper();l.Dom.clean(t);const s=new BX.TileGrid.Grid({id:"calendar_sync",items:e,container:t,sizeRatio:"55%",itemMinWidth:180,tileMargin:7,itemType:"BX.Calendar.Sync.Interface.GridUnit"});s.draw()}refresh(e,t){this.status=e;this.connectionsProviders=t;this.blockStatusContent=this.statusBlock.refresh(e,this.getConnections()).getContent();l.Dom.replace(document.querySelector("#calendar-sync-status-block"),this.blockStatusContent);this.setGridContent()}}let f=e=>e,O,b,M,w,B,P,k,W,R,U;class x{constructor(e){this.logoClassName="";this.options=e;this.connectionProvider=this.options.connectionProvider}getConnectionTemplate(){if(!this.connectionTemplate){this.connectionTemplate=this.connectionProvider.getClassTemplateItem().createInstance(this.connectionProvider)}return this.connectionTemplate}renderTo(e){if(l.Type.isElementNode(e)){e.appendChild(this.getContent())}}getContent(){this.unitNode=l.Tag.render(O||(O=f`
		...
		`));return this.unitsContentWrapper}getFooterWrapper(){return l.Tag.render(F||(F=H`
		...
		`),this.getExtraInfoWithCheckIcon(),this.getOpenAuxiliaryPanelLink(),this.getOpenHelpLink())}getExtraInfoWithCheckIcon(){const e=Object.values(this.connectionsProviders).filter((e=>e.mainPanel&&e.status)).length>0;return l.Tag.render(Y||(Y=H`
		...
		`),this.getHeaderTitle(),this.getContentInfoBody())}getContentActiveBodyHeader(){this.disconnectButton=this.getDisconnectButton();l.Event.bind(this.disconnectButton,"click",this.handleDisconnectButton.bind(this));const e=this.connection.getSyncDate().getTime()/1e3;const t=e?d.Util.formatDateUsable(e)+" "+BX.date.format(d.Util.getTimeFormatShort(),e):"";return l.Tag.render(_e||(_e=ue`
			...
			`),this.getSyncStatusClassName(),this.getLogoIconClass(),BX.util.htmlspecialchars(this.connection.getConnectionName()),t,this.disconnectButton)}getActiveConnectionContent(){return l.Tag.render(Ce||(Ce=ue`
			<div class="calendar-sync-wrap calendar-sync-wrap-detail">
				<div class="calendar-sync-header">
					<span class="calendar-sync-header-text">${0}</span>
				</div>
		...
		`),this.sliderIconClass,this.titleInfoHeader,this.descriptionInfoHeader,this.showHelp.bind(this),l.Loc.getMessage("CAL_TEXT_ABOUT_WORK_SYNC"))}getContentInfoWarning(){const e=this.getWarningButton();l.Event.bind(e,"click",this.handleWarningButtonClick.bind(this));return l.Tag.render(ye||(ye=ue`
		...
		`),this.warningText,e)}getWarningButton(){return l.Tag.render(Te||(Te=ue`
			<button class="ui-btn ui-btn-success ui-btn-sm">
				${0}
			</button>
		`),this.warningButtonText)}setProvider(e){this.provider=e}sendRequestRemoveConnection(e){BX.ajax.runAction("calendar.api.syncajax.removeConnection",{data:{connectionId:e,removeCalendars:"Y"}}).then((()=>{BX.reload()}))}runUpdateInfo(){BX.ajax.runAction("calendar.api.calendarajax.setSectionStatus",{data:{sectionStatus:this.sectionStatusObject}}).then((e=>{this.emit("reDrawCalendarGrid",{})}))}refresh(e){this.connection=e;if(this.connection){this.statusBlock.setStatus(this.connection.getStatus()).setConnections([this.connection])}l.Dom.replace(document.getElementById("status-info-block"),this.statusBlock.getContent())}handleConnectButton(){}getDisconnectButton(){return l.Tag.render(Le||(Le=ue`
			<button class="ui-btn ui-btn-light-border calendar-sync__account-btn">${0}</button>
		`),l.Loc.getMessage("CAL_SYNC_DISCONNECT_BUTTON"))}getSyncStatusClassName(){return this.provider.getStatus()==="success"?"--complete":"--error"}getContentActiveBodySectionsHeader(){return l.Tag.render(Ie||(Ie=ue`
			<div class="calendar-sync__account-desc">${0}</div>
		`),l.Loc.getMessage("CAL_SYNC_SELECTED_LIST_TITLE"))}getContentActiveBodySectionsManager(){return l.Tag.render(Ae||(Ae=ue`
			<div class="calendar-sync__account-check-list">
				${0}
			</div>
		`),this.getContentActiveBodySections())}getContentActiveBodySections(){const e=[];this.sectionList.forEach((t=>{e.push(l.Tag.render(me||(me=ue`
		...
		`),this.getTitleWrapper(),this.getSyncStagesWrapper(),this.getInfoStatusWrapper(),this.getErrorWrapper(),this.getFinalCheckWrapper(),this.getHelpLinkWrapper(),this.getButtonWrapper())}getTitleWrapper(){this.syncTitleWrapper=l.Tag.render(He||(He=Ue`
		...
		`),this.getLogoIconClass(),this.getAccountNameNode(),this.getActiveStatusNode());return this.syncTitleWrapper}getSyncStagesWrapper(){this.syncStagesWrapper=l.Tag.render($e||($e=Ue`<div class="calendar-sync-stages-wrap"></div>`));return this.syncStagesWrapper}getInfoStatusWrapper(){this.infoStatusWrapper=l.Tag.render(Xe||(Xe=Ue`
		...
		`),l.Loc.getMessage("CAL_INFO_STATUS_CONG_1"),l.Loc.getMessage("CAL_INFO_STATUS_CONG_2"));return this.infoStatusWrapper}getErrorWrapper(){this.errorWrapper=l.Tag.render(Ge||(Ge=Ue`
			<div class="calendar-sync__content-block --space-bottom-xl" style="display: none;">
		...
		`),l.Loc.getMessage("CAL_ERROR_WARN_1"),l.Loc.getMessage("CAL_ERROR_WARN_2"));return this.errorWrapper}getHelpLinkWrapper(){this.helpLinkWrapper=l.Tag.render(Fe||(Fe=Ue`
			<div class="calendar-sync__content-block" style="display: none;"></div>
		`));return this.helpLinkWrapper}getFinalCheckWrapper(){this.finalCheckWrapper=l.Tag.render(Ye||(Ye=Ue`
			<div class="calendar-sync__content-block" style="display: none;"></div>
		`));return this.finalCheckWrapper}getButtonWrapper(){this.buttonWrapper=l.Tag.render(ze||(ze=Ue`
		...
		`),l.Loc.getMessage("CAL_BUTTON_CONTINUE"),l.Loc.getMessage("CAL_BUTTON_EVERYTHING_IS_FINE"),l.Loc.getMessage("CAL_ERROR_CLOSE"));return this.buttonWrapper}getNewEventCardWrapper(){this.newEventCardWrapper=l.Tag.render(je||(je=Ue`
			<div class="calendar-sync__content-block --space-bottom" style="display: none;"></div>
		`));return this.newEventCardWrapper}getSkeletonWrapper(){this.skeletonWrapper=l.Tag.render(qe||(qe=Ue`
			<div class="calendar-sync__content-block --space-bottom">
		...
		`),this.getAppleInfoBlock(),this.getAppleIdTitle(),this.getAppleIdInput(),this.getAppleIdError(),this.getAppPasswordTitle(),this.getLearnMoreButton(),this.getAppPasswordInput(),this.getShowHidePasswordIcon(),this.getAppPasswordError());return this.DOM.container}getAppleInfoBlock(){if(!this.DOM.appleInfo){this.DOM.appleInfo=l.Tag.render(ht||(ht=lt`
			...
			`),l.Loc.getMessage("CAL_ICLOUD_INFO_BLOCK"))}return this.DOM.appleInfo}getAppleIdTitle(){if(!this.DOM.appleIdTitle){this.DOM.appleIdTitle=l.Tag.render(pt||(pt=lt`
			<p class="calendar-sync__auth-popup--label-text">
				${0}
			</p>
			`),l.Loc.getMessage("CAL_ICLOUD_APPLE_ID_PLACEHOLDER"))}return this.DOM.appleIdTitle}getAppPasswordTitle(){if(!this.DOM.appPasswordTitle){this.DOM.appPasswordTitle=l.Tag.render(ut||(ut=lt`
				<p class="calendar-sync__auth-popup--label-text">
					${0}
				</p>
			`),l.Loc.getMessage("CAL_ICLOUD_PASS_PLACEHOLDER"))}return this.DOM.appPasswordTitle}getAppleIdError(){if(!this.DOM.appleIdError){this.DOM.appleIdError=l.Tag.render(gt||(gt=lt`
				<div class="calendar-sync__auth-popup--label-text --error">
					${0}
				</div>
			`),l.Loc.getMessage("CAL_ICLOUD_APPLE_ID_ERROR"))}return this.DOM.appleIdError}getAppPasswordError(){if(!this.DOM.appPasswordError){this.DOM.appPasswordError=l.Tag.render(_t||(_t=lt`
				<div class="calendar-sync__auth-popup--label-text --error">
					${0}
				</div>
			`),l.Loc.getMessage("CAL_ICLOUD_APP_PASSWORD_ERROR",{"#LINK_START#":'<a href="#" data-role="open-helpdesk-password">',"#LINK_END#":"</a>"}));const e=this.DOM.appPasswordError.querySelector('a[data-role="open-helpdesk-password"]');if(e){l.Event.bind(e,"click",this.openHelpDesk.bind(this))}}return this.DOM.appPasswordError}getAppleIdInput(){if(!this.DOM.appleIdInput){this.DOM.appleIdInput=l.Tag.render(Ct||(Ct=lt`
				<input
					type="text"
					placeholder="${0}"
					class="calendar-field-string ui-ctl-element"
				/>
			`),l.Loc.getMessage("CAL_ICLOUD_AUTH_EMAIL_PLACEHOLDER"));this.DOM.appleIdInput.onfocus=()=>{if(l.Dom.hasClass(this.DOM.appleIdInput,"calendar-field-string-error")){l.Dom.removeClass(this.DOM.appleIdInput,"calendar-field-string-error");l.Dom.removeClass(this.DOM.appleIdError,"show")}};this.DOM.appleIdInput.onblur=()=>{if(!this.validateAppleIdInput()&&!l.Dom.hasClass(this.DOM.appleIdInput,"calendar-field-string-error")){l.Dom.addClass(this.DOM.appleIdInput,"calendar-field-string-error");l.Dom.addClass(this.DOM.appleIdError,"show")}}}return this.DOM.appleIdInput}getAppPasswordInput(){if(!this.DOM.appPasswordInput){this.DOM.appPasswordInput=l.Tag.render(Et||(Et=lt`
				<input
					type="password"
					placeholder="${0}"
					class="calendar-field-string ui-ctl-element"
		...
		`),l.Loc.getMessage("CAL_SYNC_LETS_CHECK"),l.Loc.getMessage("CAL_SYNC_CREATE_EVENT_OFFICE365"),l.Loc.getMessage("CAL_SYNC_NEW_EVENT_ADDED_FROM_OFFICE365"),l.Loc.getMessage("CAL_SYNC_NEW_EVENT_YOULL_SEE"),this.getSkeletonWrapper(),this.getNewEventCardWrapper());return this.finalCheckWrapper}setSyncStages(){this.syncStagesList=[new st({name:this.STAGE_1_CODE,title:l.Loc.getMessage("CAL_SYNC_STAGE_OFFICE365_1")}),new st({name:this.STAGE_2_CODE,title:l.Loc.getMessage("CAL_SYNC_STAGE_OFFICE365_2")}),new st({name:this.STAGE_3_CODE,title:l.Loc.getMessage("CAL_SYNC_STAGE_OFFICE365_3")})]}updateState(e){super.updateState(e);this.getSyncStages().forEach((t=>{if(e.stage==="connection_created"&&t.name===this.STAGE_1_CODE){t.setDone()}else if(e.stage===this.STAGE_2_CODE&&(t.name===this.STAGE_1_CODE||t.name===this.STAGE_2_CODE)){t.setDone()}else if(e.stage===this.STAGE_3_CODE){t.setDone();this.setActiveStatusFinished();this.showButtonWrapper();this.showInfoStatusWrapper();this.showConfetti();this.emit("onConnectionCreated")}}))}getSkeletonTitle(){return l.Loc.getMessage("CAL_SYNC_NEW_EVENT_OFFICE365_TITLE")}}class wt extends Ne{constructor(e,t=null){super({title:l.Loc.getMessage("CALENDAR_TITLE_OFFICE365"),helpDeskCode:"6030429",titleInfoHeader:l.Loc.getMessage("CAL_CONNECT_OFFICE365_CALENDAR"),descriptionInfoHeader:l.Loc.getMessage("CAL_OFFICE365_CONNECT_DESCRIPTION"),titleActiveHeader:l.Loc.getMessage("CAL_OFFICE365_CALENDAR_IS_CONNECT"),descriptionActiveHeader:l.Loc.getMessage("CAL_OFFICE365_SELECTED_DESCRIPTION"),sliderIconClass:"calendar-sync-slider-header-icon-office365",iconPath:"/bitrix/images/calendar/sync/office365.svg",iconLogoClass:"--office365",color:"#fc1d1d",provider:e,connection:t,popupWithUpdateButton:true});this.sectionStatusObject={};this.sectionList=[]}createConnection(){BX.ajax.runAction("calendar.api.calendarajax.analytical",{analyticsLabel:{calendarAction:"createConnection",click_to_connection_button:"Y",connection_type:"office365"}});BX.util.popup(this.provider.getSyncLink(),500,600);l.Event.bind(window,"hashchange",(e=>{if(window.location.hash==="#office365AuthSuccess"){d.Util.removeHash();this.provider.setWizardSyncMode(true);this.saveConnection();this.openSyncWizard();this.provider.setStatus(this.provider.STATUS_SYNCHRONIZING);this.provider.getInterfaceUnit().refreshButton()}}))}saveConnection(){return new Promise((e=>{BX.ajax.runAction("calendar.api.syncajax.createOffice365Connection").then((t=>{var s,n;if((t==null?void 0:(s=t.data)==null?void 0:s.status)===this.provider.ERROR_CODE){this.provider.setStatus(this.provider.STATUS_FAILED);this.provider.setWizardState({status:this.provider.ERROR_CODE,vendorName:this.provider.type})}else if(t!=null&&(n=t.data)!=null&&n.connectionId){this.provider.setStatus(this.provider.STATUS_SUCCESS);this.provider.getConnection().setId(t.data.connectionId);this.provider.getConnection().setStatus(true);this.provider.getConnection().setConnected(true);this.provider.getConnection().setSyncDate(new Date)}e(t.data)}),(t=>{this.provider.setStatus(this.provider.STATUS_FAILED);this.provider.setWizardState({status:this.provider.ERROR_CODE,vendorName:this.provider.type});e(t.errors)}))}))}onClickCheckSection(e){this.sectionStatusObject[e.target.value]=e.target.checked;this.runUpdateInfo();this.showUpdateSectionListNotification()}handleConnectButton(){if(this.provider.hasSetSyncOffice365Settings()){this.createConnection()}else{this.showAlertPopup()}}openSyncWizard(){this.wizard=new Mt;this.wizard.openSlider();this.provider.setActiveWizard(this.wizard)}getSectionsForOffice365(){return new Promise((e=>{BX.ajax.runAction("calendar.api.syncajax.getAllSectionsForOffice365",{data:{connectionId:this.connection.addParams.id}}).then((t=>{this.sectionList=t.data;e(t.data)}),(t=>{e(t.errors)}))}))}sendRequestRemoveConnection(e){this.deactivateConnection(e)}showAlertPopup(){const e=new c.MessageBox({className:this.id,message:l.Loc.getMessage("OFFICE365_IS_NOT_CALDAV_SETTINGS_WARNING_MESSAGE"),width:500,offsetLeft:60,offsetTop:5,padding:7,onOk:()=>{e.close()},okCaption:"OK",buttons:BX.UI.Dialogs.MessageBoxButtons.OK,popupOptions:{zIndexAbsolute:4020,autoHide:true}});e.show()}}let Bt=e=>e,Pt,kt,Wt;class Rt extends Ne{constructor(e,t=null){super({title:l.Loc.getMessage("CALENDAR_TITLE_MAC"),helpDeskCode:"5684075",titleInfoHeader:l.Loc.getMessage("CAL_CONNECT_MAC_CALENDAR_TITLE"),descriptionInfoHeader:l.Loc.getMessage("CAL_MAC_CONNECT_DESCRIPTION"),titleActiveHeader:l.Loc.getMessage("CAL_MAC_CALENDAR_IS_CONNECT_TITLE"),descriptionActiveHeader:l.Loc.getMessage("CAL_MAC_SELECTED_DESCRIPTION"),sliderIconClass:"calendar-sync-slider-header-icon-mac",iconPath:"/bitrix/images/calendar/sync/mac.svg",color:"#ff5752",provider:e,connection:t,popupWithUpdateButton:false});this.warningText=l.Loc.getMessage("CAL_SYNC_WARNING_IPHONE_AND_MAC");this.warningButtonText=l.Loc.getMessage("CALENDAR_CONNECT_ICLOUD")}getPortalAddress(){return this.portalAddress}getContentInfoBody(){return l.Tag.render(Pt||(Pt=Bt`
			${0}
			${0}
			${0}
		`),this.getContentInfoBodyHeader(),this.getContentInfoWarning(),this.getContentBodyConnect())}getContentActiveBody(){return l.Tag.render(kt||(kt=Bt`
			${0}
			${0}
		`),this.getContentActiveBodyHeader(),this.getContentBodyConnect())}getContentBodyConnect(){return l.Tag.render(Wt||(Wt=Bt`
		...
		`),l.Loc.getMessage("CAL_MAC_INSTRUCTION_HEADER"),l.Loc.getMessage("CAL_MAC_INSTRUCTION_DESCRIPTION"),l.Loc.getMessage("CAL_MAC_INSTRUCTION_POINT_FIRST"),l.Loc.getMessage("CAL_MAC_INSTRUCTION_POINT_SECOND"),l.Loc.getMessage("CAL_MAC_INSTRUCTION_POINT_THIRD"),l.Loc.getMessage("CAL_MAC_INSTRUCTION_POINT_FOURTH"),l.Loc.getMessage("CAL_MAC_INSTRUCTION_POINT_FIFTH",{"#PORTAL_ADDRESS#":this.provider.getPortalAddress()}),l.Loc.getMessage("CAL_MAC_INSTRUCTION_POINT_SIXTH"),l.Loc.getMessage("CAL_MAC_INSTRUCTION_POINT_SEVENTH"),l.Loc.getMessage("CAL_MAC_INSTRUCTION_CONCLUSION"))}handleWarningButtonClick(){BX.SidePanel.Instance.getOpenSliders().forEach((e=>{if(["calendar:auxiliary-sync-slider","calendar:item-sync-connect-mac"].includes(e.getUrl())){e.close()}}));const e=d.Util.getCalendarContext();if(e){e.syncInterface.getIcloudProvider().getInterfaceUnit().getConnectionTemplate().handleConnectButton()}}}class Ut extends Ne{constructor(e,t=null){super({title:l.Loc.getMessage("CALENDAR_TITLE_MAC"),helpDeskCode:"5684075",titleInfoHeader:l.Loc.getMessage("CAL_CONNECT_MAC_CALENDAR_TITLE"),descriptionInfoHeader:l.Loc.getMessage("CAL_MAC_CONNECT_DESCRIPTION"),titleActiveHeader:l.Loc.getMessage("CAL_MAC_CALENDAR_IS_CONNECT_TITLE"),descriptionActiveHeader:l.Loc.getMessage("CAL_MAC_SELECTED_DESCRIPTION"),sliderIconClass:"calendar-sync-slider-header-icon-mac",iconPath:"/bitrix/images/calendar/sync/mac.svg",color:"#ff5752",provider:e,connection:t,popupWithUpdateButton:false})}}class xt extends be{constructor(e,t=null){super({title:l.Loc.getMessage("CALENDAR_TITLE_YANDEX"),helpDeskCode:"10930170",titleInfoHeader:l.Loc.getMessage("CAL_CONNECT_YANDEX_CALENDAR"),descriptionInfoHeader:l.Loc.getMessage("CAL_YANDEX_CONNECT_DESCRIPTION"),titleActiveHeader:l.Loc.getMessage("CAL_YANDEX_CALENDAR_IS_CONNECT"),descriptionActiveHeader:l.Loc.getMessage("CAL_YANDEX_SELECTED_DESCRIPTION"),sliderIconClass:"calendar-sync-slider-header-icon-yandex",iconPath:"/bitrix/images/calendar/sync/yandex.svg",iconLogoClass:"--yandex",color:"#f9c500",provider:e,connection:t,popupWithUpdateButton:true})}}let Ht=e=>e,$t,Xt,Gt,Ft;class Yt extends Ne{constructor(e){super(e);this.banner=new pe({type:this.provider.getType(),helpDeskCode:e.helpDeskCode});if(this.status){this.syncDate=l.Type.isDate(this.data.syncDate)?this.data.syncDate:d.Util.parseDate(this.data.syncDate)}}getContentInfoBody(){return l.Tag.render($t||($t=Ht`
			${0}
			${0}
			<div class="calendar-sync-slider-section calendar-sync-slider-section-banner">
				${0}
			</div>
		`),this.getContentInfoBodyHeader(),this.getContentInfoWarning(),this.getContentBodyConnect())}getContentActiveBody(){return l.Tag.render(Xt||(Xt=Ht`
			${0}
			<div class="calendar-sync-slider-section calendar-sync-slider-section-banner">
				${0}
			</div>
		`),this.getContentActiveBodyHeader(),this.getContentBodyConnect())}getContentActiveBodyHeader(){const e=this.connection.getSyncDate().getTime()/1e3;const t=e?d.Util.formatDateUsable(e)+" "+BX.date.format(d.Util.getTimeFormatShort(),e):"";return l.Tag.render(Gt||(Gt=Ht`
			<div class="calendar-sync-slider-section">
				<div class="calendar-sync-slider-header-icon ${0}"></div>
				<div class="calendar-sync-slider-header">
			...
			</div>`),this.sliderIconClass,this.titleActiveHeader,l.Loc.getMessage("CAL_SYNC_LAST_SYNC_DATE"),t,l.Loc.getMessage("CAL_SYNC_DISABLE"),this.showHelp.bind(this),l.Loc.getMessage("CAL_TEXT_ABOUT_WORK_SYNC"))}getContentBodyConnect(){this.banner.initQrCode().then(this.banner.drawQRCode.bind(this.banner));return this.banner.getContainer()}getActiveConnectionContent(){return l.Tag.render(Ft||(Ft=Ht`
			<div class="calendar-sync-wrap calendar-sync-wrap-detail">
				<div class="calendar-sync-header">
					<span class="calendar-sync-header-text">${0}</span>
				</div>
		...
		`),this.getHeaderTitle(),this.getContentActiveBody())}}class zt extends Yt{constructor(e,t=null){super({title:l.Loc.getMessage("CALENDAR_TITLE_ANDROID"),helpDeskCode:"5686179",titleInfoHeader:l.Loc.getMessage("CAL_CONNECT_ANDROID_CALENDAR_TITLE"),descriptionInfoHeader:l.Loc.getMessage("CAL_ANDROID_CONNECT_DESCRIPTION"),titleActiveHeader:l.Loc.getMessage("CAL_SYNC_CONNECTED_ANDROID_TITLE"),descriptionActiveHeader:l.Loc.getMessage("CAL_ANDROID_SELECTED_DESCRIPTION"),sliderIconClass:"calendar-sync-slider-header-icon-android",iconPath:"/bitrix/images/calendar/sync/android.svg",color:"#9ece03",provider:e,connection:t,popupWithUpdateButton:false});this.warningText=l.Loc.getMessage("CAL_SYNC_WARNING_ANDROID");this.warningButtonText=l.Loc.getMessage("CALENDAR_CONNECT_GOOGLE")}handleWarningButtonClick(){BX.SidePanel.Instance.getOpenSliders().forEach((e=>{if(["calendar:auxiliary-sync-slider","calendar:item-sync-connect-android"].includes(e.getUrl())){e.close()}}));const e=d.Util.getCalendarContext();if(e){e.syncInterface.getGoogleProvider().getInterfaceUnit().getConnectionTemplate().handleConnectButton()}}}class jt extends Yt{constructor(e,t=null){super({title:l.Loc.getMessage("CALENDAR_TITLE_IPHONE"),helpDeskCode:"5686207",titleInfoHeader:l.Loc.getMessage("CAL_CONNECT_IPHONE_CALENDAR_TITLE"),descriptionInfoHeader:l.Loc.getMessage("CAL_IPHONE_CONNECT_DESCRIPTION"),titleActiveHeader:l.Loc.getMessage("CAL_SYNC_CONNECTED_IPHONE_TITLE"),descriptionActiveHeader:l.Loc.getMessage("CAL_IPHONE_SELECTED_DESCRIPTION"),sliderIconClass:"calendar-sync-slider-header-icon-iphone",iconPath:"/bitrix/images/calendar/sync/iphone.svg",color:"#2fc6f6",provider:e,connection:t,popupWithUpdateButton:false});this.warningText=l.Loc.getMessage("CAL_SYNC_WARNING_IPHONE_AND_MAC");this.warningButtonText=l.Loc.getMessage("CALENDAR_CONNECT_ICLOUD")}handleWarningButtonClick(){BX.SidePanel.Instance.getOpenSliders().forEach((e=>{if(["calendar:auxiliary-sync-slider","calendar:item-sync-connect-iphone"].includes(e.getUrl())){e.close()}}));const e=d.Util.getCalendarContext();if(e){e.syncInterface.getIcloudProvider().getInterfaceUnit().getConnectionTemplate().handleConnectButton()}}}let qt=e=>e,Kt,Vt;var Qt=babelHelpers.classPrivateFieldLooseKey("showSuccessCopyNotification");var Zt=babelHelpers.classPrivateFieldLooseKey("showFailedCopyNotification");var Jt=babelHelpers.classPrivateFieldLooseKey("showResultNotification");class es{constructor(e){Object.defineProperty(this,Jt,{value:ns});Object.defineProperty(this,Zt,{value:ss});Object.defineProperty(this,Qt,{value:ts});this.LINK_LENGTH=112;this.link=this.getIcalLink(e)}static createInstance(e){return new this(e)}show(){this.createPopup().show();this.startSync()}startSync(){BX.ajax.get(this.link+"&check=Y","",(e=>{setTimeout((()=>{if(!e||e.length<=0||e.toUpperCase().indexOf("BEGIN:VCALENDAR")===-1){this.showPopupWithSyncDataError()}}),300)}))}getContent(){return l.Tag.render(Kt||(Kt=qt`
			<div class="calendar-ical-popup-wrapper">
				<h3>${0}</h3>
				<div class="calendar-ical-popup-label-text"><span>${0}</span></div>
				${0}
			</div>
		`),l.Loc.getMessage("EC_JS_EXPORT_TILE"),l.Loc.getMessage("EC_EXP_TEXT"),this.getLinkBlock())}createPopup(){return this.popup=new h.Popup({width:400,zIndexOptions:4e3,autoHide:false,closeByEsc:true,draggable:true,closeIcon:{right:"12px",top:"10px"},className:"bxc-popup-window",content:this.getContent(),buttons:[new BX.UI.Button({text:l.Loc.getMessage("EC_JS_ICAL_COPY_ICAL_SYNC_LINK"),color:BX.UI.Button.Color.PRIMARY,onclick:()=>{this.copyLink(event)}}),new BX.UI.Button({text:l.Loc.getMessage("EC_SEC_SLIDER_CLOSE"),color:BX.UI.Button.Color.LINK,onclick:()=>{this.popup.close()}})]})}getIcalLink(e){return e.calendarPath+(e.calendarPath.indexOf("?")>=0?"&":"?")+"action=export"+e.sectionLink}getLinkBlock(){return l.Tag.render(Vt||(Vt=qt`
				<div class="calendar-ical-popup-link-block">
					<a class="ui-link ui-link-primary " target="_blank" href="${0}">
						${0}
					</a>
				</div>
			`),BX.util.htmlspecialchars(this.link),BX.util.htmlspecialchars(this.getShortenLink(this.link)))}static checkPathes(e){return!!e.sectionLink||!!e.calendarPath}static showPopupWithPathesError(){BX.UI.Dialogs.MessageBox.alert(l.Loc.getMessage("EC_JS_ICAL_ERROR_WITH_PATHES"))}showPopupWithSyncDataError(){BX.UI.Dialogs.MessageBox.alert(l.Loc.getMessage("EC_EDEV_EXP_WARN"))}copyLink(e){window.BX.clipboard.copy(this.link)?babelHelpers.classPrivateFieldLooseBase(this,Qt)[Qt]():babelHelpers.classPrivateFieldLooseBase(this,Zt)[Zt]();e.preventDefault();e.stopPropagation()}getShortenLink(e){return e.length<this.LINK_LENGTH?e:e.substr(0,105)+"..."+e.slice(-7)}}function ts(){babelHelpers.classPrivateFieldLooseBase(this,Jt)[Jt](l.Loc.getMessage("EC_JS_ICAL_COPY_ICAL_SYNC_LINK_SUCCESS"))}function ss(){babelHelpers.classPrivateFieldLooseBase(this,Jt)[Jt](l.Loc.getMessage("EC_JS_ICAL_COPY_ICAL_SYNC_LINK_FAILED"))}function ns(e){d.Util.showNotification(e)}class is{constructor(e={}){this.options=e}static createInstance(e){return new this(e)}loadExtension(){return new Promise((e=>{l.Runtime.loadExtension("ui.tour").then((t=>{if(t&&t["Guide"]&&t["Manager"]){e()}else{console.error(`Extension "ui.tour" not found`)}}))}))}show(){this.loadExtension().then((()=>{this.guide=new BX.UI.Tour.Guide({steps:[{target:this.getTarget(),title:l.Loc.getMessage("CAL_AFTER_SYNC_AHA_TITLE"),text:l.Loc.getMessage("CAL_AFTER_SYNC_AHA_TEXT")}],onEvents:true});this.guide.start()}))}getTarget(){let e;const t=this.options.view;const s=t.getContainer();if(t.getName()==="month"){e=s.querySelectorAll(".calendar-grid-today")[0]}else if(t.getName()==="day"||t.getName()==="week"){const t=d.Util.getDayCode(new Date);e=s.querySelector('div[data-bx-calendar-timeline-day="'+t+'"] .calendar-grid-cell-inner')}else{e=document.querySelector('span[data-role="addButton"]')}return e}}e.SyncPanel=q;e.SyncPanelUnit=x;e.AuxiliarySyncPanel=D;e.GridUnit=ee;e.ConnectionControls=ce;e.MobileSyncBanner=pe;e.YandexTemplate=xt;e.CaldavTemplate=Me;e.MacTemplate=Rt;e.ExchangeTemplate=Re;e.GoogleTemplate=ct;e.IcloudTemplate=ft;e.OutlookTemplate=Ut;e.IphoneTemplate=jt;e.AndroidTemplate=zt;e.IcalSyncPopup=es;e.AfterSyncTour=is;e.GoogleSyncWizard=rt;e.Office365template=wt;e.IcloudAuthDialog=Tt})(this.BX.Calendar.Sync.Interface=this.BX.Calendar.Sync.Interface||{},BX,BX,BX.Calendar.Sync.Manager,BX.Calendar,BX,BX,BX.Event,BX.UI.Dialogs,BX,BX.Calendar,BX.Main);
